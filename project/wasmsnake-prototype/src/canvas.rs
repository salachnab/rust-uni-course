...
const BACKGROUND_COLOUR: &str = "white";
...
impl Canvas {
    ...
    //draws one square of given colour
    pub fn draw(&self, x: u32, y: u32, colour: &str) {
        assert!(x < self.width);
        assert!(y < self.height);

        self.ctx.set_fill_style_color(colour);
...
/*
    pub fn draw_menu(&self, x: u32, y: u32, colour: &str) {
        assert!(x < self.width);
        assert!(y < self.height);
    }
*/
    pub fn clear_all(&self) {
        self.ctx.set_fill_style_color(BACKGROUND_COLOUR);
        self.ctx.fill_rect(
            0.0,
            0.0,
